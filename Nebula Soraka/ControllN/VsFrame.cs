    ...
    public class VsFrame : ValueBase<Color>
    {
        ...
        public VsFrame(string uId, Color defaultValue) : base(uId, "", 265)
        {
            ImageSprite = new Sprite(
                Texture.FromMemory(
                    Drawing.Direct3DDevice, (byte[])new ImageConverter().ConvertTo(Properties.Resources.Img_Soraka, typeof(byte[])),
        ...
        public override bool Draw()
        {
            if (MainMenu.IsVisible && IsVisible)
            {
                try
                {
                    ImageSprite.Draw(new Vector2(Soraka.Menu["Language.Select"].Position.X - 14, Soraka.Menu["Language.Select"].Position.Y + 185));
                }
                catch
                {
                }
