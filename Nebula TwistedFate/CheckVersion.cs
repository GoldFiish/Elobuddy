...
namespace NebulaTwistedFate
{
    class CheckVersion : TwistedFate
    {
        ...
        {
            WebRequest Request_GitHubVer = WebRequest.Create("https://github.com/GoldFiish/Elobuddy/blob/master/Nebula%20Soraka/TFateVersion.txt");
            using (var Version_Response = (HttpWebResponse)Request_GitHubVer.GetResponse())
            {
                ...
                {
                    Chat.Print("<font color = '#ffffff'>[ Notice ] </font><font color = '#ebfd00'>Nebula TwistedFate has been Update </font><font color = '#ffffff'>" + NoticeList[1] + "</font>");
